-- Seoul Sister Premium Intelligence Platform - Price Comparison Engine
-- New tables for real-time price tracking and deal discovery
...
CREATE TABLE IF NOT EXISTS price_retailers (
    ...
    country VARCHAR(10) NOT NULL, -- US, KR, etc
    ...
    -- Reliability metrics
    success_rate DECIMAL(5,2) DEFAULT 100.00, -- % successful scrapes
...
CREATE TABLE IF NOT EXISTS product_prices (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    product_id VARCHAR(255) NOT NULL, -- References your existing products
    retailer_id UUID NOT NULL REFERENCES price_retailers(id),

    -- Price information
    current_price DECIMAL(10,2) NOT NULL,
    original_price DECIMAL(10,2), -- MSRP or original listing price
    ...
    total_cost DECIMAL(10,2) GENERATED ALWAYS AS (current_price + COALESCE(shipping_cost, 0)) STORED,

    -- Data quality
    price_confidence DECIMAL(3,2) DEFAULT 1.00, -- How confident we are in this price
    data_source VARCHAR(50) DEFAULT 'scraping', -- scraping, api, manual
...
CREATE TABLE IF NOT EXISTS price_history (
    ...
    sale_type VARCHAR(50), -- flash_sale, clearance, seasonal, etc

    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

    -- Unique constraint
    UNIQUE(product_id, retailer_id, recorded_date)
);
...
CREATE TABLE IF NOT EXISTS daily_deals (
    ...
    deal_score INTEGER CHECK (deal_score >= 0 AND deal_score <= 100), -- Algorithm-generated deal quality score
    ...
    -- Unique constraint: one deal per product per retailer per day
    UNIQUE(product_id, retailer_id, deal_date)
);

-- ============================================
-- PART 5: User Watchlists & Personal Alerts
-- ============================================
...
-- ============================================
-- PART 6: Premium Member Subscriptions
-- ============================================
...
CREATE TABLE IF NOT EXISTS premium_subscriptions (
    ...
    -- Plan information
    plan_type VARCHAR(50) DEFAULT 'premium' CHECK (plan_type IN ('premium', 'premium_plus')),
...
-- ============================================
-- PART 7: Korean Supplier Directory
-- ============================================
...
CREATE TABLE IF NOT EXISTS korean_suppliers (
    ...
    district VARCHAR(100), -- Gangnam, Hongdae, etc
    address TEXT,

    -- Business details
    specialties TEXT[], -- skincare, makeup, tools, etc
    minimum_order_usd DECIMAL(10,2),
    payment_methods TEXT[], -- wire_transfer, paypal, etc
    shipping_methods TEXT[], -- ems, dhl, fedex, etc
    ...
    -- Performance metrics
    response_rate DECIMAL(3,2) DEFAULT 0.95, -- How often they respond
...
-- ============================================
-- PART 8: Group Buying Coordination
-- ============================================
...
CREATE TABLE IF NOT EXISTS group_buys (
    ...
    -- Product information
    product_id VARCHAR(255), -- If specific product
    supplier_id UUID REFERENCES korean_suppliers(id),
    product_category VARCHAR(100),

    -- Pricing tiers
    individual_price DECIMAL(10,2),
    group_price_10 DECIMAL(10,2), -- Price for 10+ units
    group_price_25 DECIMAL(10,2), -- Price for 25+ units
    group_price_50 DECIMAL(10,2), -- Price for 50+ units

    -- Requirements
    minimum_participants INTEGER DEFAULT 10,
    maximum_participants INTEGER DEFAULT 100,
    minimum_order_per_person INTEGER DEFAULT 1,
    ...
    -- Status
    status VARCHAR(20) DEFAULT 'open' CHECK (status IN ('planning', 'open', 'closed', 'paid', 'shipped', 'completed', 'canceled')),

    -- Current stats
    current_participants INTEGER DEFAULT 0,
    total_units_committed INTEGER DEFAULT 0,
    total_value_usd DECIMAL(12,2) DEFAULT 0,

    -- Organizer
    organizer_id UUID, -- Admin user who organized this
...
CREATE TABLE IF NOT EXISTS group_buy_participants (
    ...
    shipping_preference VARCHAR(50), -- individual, consolidated, pickup
...
-- Unique constraint for one price per product per retailer per day
CREATE UNIQUE INDEX IF NOT EXISTS idx_product_prices_unique_daily
ON product_prices(product_id, retailer_id, (price_date::date));
...
CREATE INDEX IF NOT EXISTS idx_price_history_lowest ON price_history(is_lowest_price) WHERE is_lowest_price = true;
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_trial_end ON premium_subscriptions(trial_end);
...
CREATE INDEX IF NOT EXISTS idx_group_buys_deadline ON group_buys(signup_deadline);
CREATE INDEX IF NOT EXISTS idx_group_buy_participants_group_id ON group_buy_participants(group_buy_id);

-- ============================================
-- PART 10: Insert Initial Data
-- ============================================
...
-- Insert sample Korean suppliers
...
-- Price Intelligence Platform database setup complete!
-- Ready for real-time price tracking and premium member features.