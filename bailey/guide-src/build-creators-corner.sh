...
# (PDF + DOCX) for Bailey.
#
# Source: ../CREATORS-CORNER-AND-WHATS-WORKING-JUL30.md — like the frequency and
...
# Output: bailey/Creators-Corner-Jul30.{pdf,docx} + ~/Downloads copies
#
...
SRC="../CREATORS-CORNER-AND-WHATS-WORKING-JUL30.md"
REPO_OUT="$(cd .. && pwd)/Creators-Corner-Jul30"
OUT="$HOME/Downloads/Creators-Corner-Jul30"
...
pandoc "$SRC" -o /tmp/ccorner.html --standalone -c style.css

python3 - <<'PY'
html = open('/tmp/ccorner.html').read()
...
open('/tmp/ccorner_inline.html', 'w').write(html)
PY

if [ -x "$CHROME" ]; then
  "$CHROME" --headless --disable-gpu --no-pdf-header-footer \
    --print-to-pdf="$OUT.pdf" "file:///tmp/ccorner_inline.html" 2>/dev/null
  echo "wrote $OUT.pdf"
