...
**What shipped**: the interlocked SS monogram, built from Playfair Display glyph **outlines** — a high-contrast serif, which is the register her Canva draft was reaching for. Two members of one family: `icon-tile` (dark serif knocked out of a gold gradient tile) for the app icon and favicon, and `ss-mark-gold` (gold metallic serif on near-black) for large-format use. Her interlocking instinct is the core of it — the two S's overlap 12% of the em so they read as ONE designed mark rather than typed letters. The ring variant was **killed**: its hairline circle evaporates below ~128px.

**Three standing decisions from an adversarial design review (record these, they are not obvious):**

1. **Bailey's TikTok avatar must stay HER FACE — not the monogram.** The mark is legible at 40px in a comment thread, so this is not a technical limit. On a personal creator account a logo avatar reads *brand-shill* and discounts the content, which is exactly the authenticity her channel runs on. The monogram belongs in her banner, a pinned video, or a watermark corner. **This is the one use I was wrong about and it was caught before anyone acted on it.**
2. **This is v1, and the concept is right while the execution is rentable.** Playfair Display is the most-defaulted "elegant" Google Font — a designer will clock it as template. Budget a custom redraw of the SAME concept later: a true over-under weave (nothing currently passes behind anything, so the crossing reads as a registration error to a trained eye) and hairlines thickened ~15–20% so a 16px favicon does not rot. Do NOT start a new concept.
3. **The S's stay curvaceous serif forms forever.** A stylized "SS" has one catastrophic prior association (the Nazi *Schutzstaffel* sig-runes). Curvy high-contrast serifs are nowhere near those angular lightning forms, so present risk is low — but any future "modernizing" restyle toward angular or rune-like strokes is banned. Germany has pulled merchandise over stylized-SS marks. Also: the monogram *alone* is weakly protectable ("SS" marks are common), so clear a USPTO/EUIPO knockout search in cosmetics class 3/44 before spending on merch.

**Positioning accepted on the record**: a gold serif monogram sits in the Sulwhasoo/premium register, NOT the Glossier/Rhode/Byoma pastel-minimal one. That is coherent with a dark+gold site at $24.99 on the teledermatology shelf — but it is a choice, not a default.
