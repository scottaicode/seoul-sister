...
| **`BAILEY-SCRIPTS-SEARCH-FIRST-JUL30.md`** + `Bailey-Scripts-Search-First-Jul30.{pdf,docx}` | **Send-ready.** 10 scripts (5 sunscreen-reformulation, 5 app-demo/authority), all **search-phrase-first** — TikTok search is now a primary discovery surface, so the phrase is spoken in the first 2s, on screen, and opens the caption. Every fact verified against the live DB before writing: BoJ Relief Sun **is** in Bailey's routine (SUN-4 is true as written), Retinol+Glycolic **is** a real `high`-severity conflict row (SS-2's demo works), counts re-checked (6,065/553/14,961). Built by `guide-src/build-search-first-scripts.sh` |
| **`CREATORS-CORNER-AND-WHATS-WORKING-JUL30.md`** + `Creators-Corner-Jul30.{pdf,docx}` | **Send-ready.** Jul 30 answer to "is the Creator's Corner mentorship worth it?" Verdict: **no** — $3.5-4K non-refundable, ~7h video, member testimonials are referral-incentivized (Whop pays 30% by default), and it teaches TikTok Shop selling whose **beauty commissions were roughly halved June 22 2026**. Also carries the two findings she didn't ask for: her **F4F / follower-goal content is actively harming reach** under 2026 follower-first test distribution, and the **Olive Young→Sephora fall rollout** opens a ~90-day window for the US-vs-Korean sunscreen reformulation angle (a fact this repo has verified and almost no creator is covering). Built by `guide-src/build-creators-corner.sh` |
