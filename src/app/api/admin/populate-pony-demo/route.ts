...
export async function POST(request: NextRequest) {
  try {
    ...
    if (influencerError) {
      ...
      return NextResponse.json({ error: 'Failed to create influencer', details: influencerError.message }, { status: 500 })
    }
    ...
    if (postsError) {
      ...
      return NextResponse.json({ error: 'Failed to insert posts', details: postsError.message }, { status: 500 })
    }
    ...
    if (productsError) {
      ...
      return NextResponse.json({ error: 'Failed to insert products', details: productsError.message }, { status: 500 })
    }
    ...
    return NextResponse.json({
      success: true,
      message: '@ponysmakeup demo data populated successfully',
      data: {
    ...
    return NextResponse.json(
      { error: 'Failed to populate demo data', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}