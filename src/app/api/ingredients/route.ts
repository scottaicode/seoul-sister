...
export async function GET(request: NextRequest) {
  try {
    ...
    // Never surface unsplit INCI dumps. Shared guard — see
    // excludePollutedIngredientRows() in the parser module for the rationale;
    // every ss_ingredients read path must go through it (July 12 audit found
    // this filter hand-rolled here while three sibling read paths had none).
    query = excludePollutedIngredientRows(query)
