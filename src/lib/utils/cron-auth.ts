...
export function verifyCronAuth(request: Request): NextResponse | null {
  ...
  // DEBUG: Temporary logging to diagnose 401 issue — remove after fix
  console.log(`[cron-auth] provided length=${provided.length}, secret length=${secret.length}, match=${provided === secret}`)
  console.log(`[cron-auth] provided first4="${provided.slice(0, 4)}" last4="${provided.slice(-4)}", secret first4="${secret.slice(0, 4)}" last4="${secret.slice(-4)}"`)

  if (provided.length !== secret.length) {
    console.log(`[cron-auth] REJECTED: length mismatch`)
  ...
  if (!timingSafeEqual(a, b)) {
    console.log(`[cron-auth] REJECTED: value mismatch`)
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }
